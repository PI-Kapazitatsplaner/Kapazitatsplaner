  ...
  for (const team of currentUserTeams) {
    ...
    for(const sprint of sprintsInPi){  
      ...
      for (const member of teamMembers) {
        ...
        daysWithUserProductivity += Math.round(usersDaysInSprint * (team.productivityPercentage / 100) * 2) / 2;
      }    
    ...
    const umgesetzteStorypoints: number[] = kapazitaetProSprint.map(a => a + 1); 
