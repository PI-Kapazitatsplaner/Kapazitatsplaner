...
router.get("/:year?/:month?", async (req, res) => {
  ...
  if (validateParams(req.params)) {
    const abwesenheitenInMonth = await getAbwesenheitenInMonth(
      Number(year),
      Number(month),
      req.user.sub,
    );
    ...
    const calendar = {
      ...
      daysInMonth: new Date(date.getFullYear(), date.getMonth() + 1, 0)
        .getDate(),
      activeDay: !(Number(year) === new Date().getFullYear() &&
    ...
    if (year !== undefined && Number(year) >= 2020 && Number(year) < 2100) {
      ...
      res.redirect(
        "/mein_kalender/" + new Date().getFullYear() + "/" +
          (new Date().getMonth() + 1),
      );
    }
  }
...
function validateBody(body: any): Boolean {
  return body.newlyChangedAbwesenheiten !== undefined &&
    body.newlyChangedAbwesenheiten.length > 0;
}

async function getAbwesenheitenInMonth(
  year: number,
  month: number,
  userSub: string,
      ...
      where: {
        ...
        AND: {
          date: {
            ...
            lt: new Date(
              (month >= 12 ? 0 : year) + "-" + (month >= 12 ? 1 : (month + 1)) +
                "-" + "1",
            ),
          },
        },
...
async function swtichAbwesenheitsType(
  newState: string,
  year: number,
  month: number,
  day: number,
  ...
  if (
    newState === "anwesend" &&
    !req.user.standardAbwesenheiten.includes(
      new Date(Number(year), Number(month) - 1, day).getDay(),
    )
  ) {
    try {
